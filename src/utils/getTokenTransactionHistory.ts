...
export const getUnconfirmedTxs = async (slpAddresses: any) => {
  // const lastTransactions = await SLP.Address.transactions(slpAddresses);
  const lastTransactions = await electrumx.getTransactions(slpAddresses[0]);
...
export const getConfirmedSlpTxsByTokenId = async (slpAddresses: any, tokenId: any) => {
  // const transactions = await SLP.Utils.bulkTransactions(slpAddresses.map((address) => ({ tokenId, address })));
  const transactions: any = [];
  return transactions;
};

export const getAllConfirmedSlpTxs = async (slpAddresses: any, tokens: any) => {
...
const getTokenTransactionHistory = async (slpAddresses: any, tokenInfo: any, tokenUtxos: any) => {
  try {
    const { tokenId } = tokenInfo;

    const Xor = (x: any, y: any) => (x || y) && !(x && y);

    const calculateTransactionBalance = (outputs: any) => {
      if (outputs.length === 1 && slpAddresses.includes(outputs[0].address)) return +outputs[0].amount;
      if (outputs.length === 1 && !slpAddresses.includes(outputs[0].address)) return +outputs[0].amount * -1;
      ...
      if (outputs.length > 1 && outputs.findIndex((element: any) => slpAddresses.includes(element.address)) !== -1)
        return +outputs.find((element: any) => slpAddresses.includes(element.address)).amount;
    };
        ...
        .filter(
          (tx: any, index: number, array: any) =>
            !(
              ...
              array[index + 1].detail.outputs.findIndex((output: any) => slpAddresses.includes(output.address)) === -1
            )
        );
    ...
    const transactions: any = await getConfirmedSlpTxsByTokenId(slpAddresses, tokenId);
