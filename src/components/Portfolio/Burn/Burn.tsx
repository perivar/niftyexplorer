...
const validateAmount = (amount: any, tokenBalance: any, step: any, decimals: any) => {
  const valid =
    amount !== '' &&
...
const Burn = ({ token, avatar, onClose }: any) => {
  ...
  const handleChangeSwitch = (checked: any) => setFormData((p) => ({ ...p, burnBaton: checked }));
                        ...
                        <ul>
                          ...
                          {hasBaton && (
                            <li>
                              <Paragraph>
                                <strong>
                                  If the <em>"Burn Mint Baton?"</em> option is checked you will LOSE the ability to mint
                                  more <em>{token.info.name}</em> tokens permanently.
                                </strong>
                              </Paragraph>
                            </li>
                          )}
                ...
                <Form style={{ width: 'auto' }}>
                  {hasBalance && (
                    <FormItemWithMaxAddon
                      validateStatus={
                        !formData.dirty && !validateAmount(formData.amount, token.balance.toString(), step, decimals)
                          ? 'error'
                          : ''
                      }
                      help={
                  ...
                  {hasBaton && (
                    <Form.Item
                      required={!hasBalance}
                      validateStatus={!formData.dirty && formData.burnBaton === false && !hasBalance ? 'error' : ''}
                      help={
                      ...
                      <StyledSwitch>
                        <Switch style={{ color: '#F34745' }} onChange={(e) => handleChangeSwitch(e)} /> Burn Mint Baton?
                      </StyledSwitch>
                    </Form.Item>
                  )}
                  ...
                  <StyledInput>
                    <Form.Item
                      validateStatus={
                        !formData.dirty && formData.textToDelete !== `burn ${token.info.name}` ? 'error' : ''
                      }
                      help={
                  ...
                  <div style={{ paddingTop: '12px' }}>
                    <Button style={{ color: '#f34745', borderColor: '#f34745' }} type="link" onClick={() => submit()}>
                      <FireIcon />
                      Burn
                    </Button>
                  </div>
